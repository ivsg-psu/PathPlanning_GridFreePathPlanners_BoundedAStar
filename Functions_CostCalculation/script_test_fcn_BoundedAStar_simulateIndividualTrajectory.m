% script_test_fcn_BoundedAStar_simulateIndividualTrajectory
% Tests: fcn_BoundedAStar_simulateIndividualTrajectory

% Revision history
% 2025_08_08 by K. Hayes, kxh1031@psu.edu
...
%% DEMO case: do an open loop simulation of a point-to-point trajectory
fig_num = 10001;
titleString = sprintf('DEMO case: do an open loop simulation of a point-to-point trajectory');
...
% Put a point into the wind field and show the result of trying a straight
% line trajectory
startPoint = [8 -6];
finishPoint = [-8 -6];

% Generate input trajectory
distance = sum((startPoint - finishPoint).^2,2).^0.5;
angle = atan([finishPoint(2)-startPoint(2)]/[finishPoint(1)-startPoint(1)]);
radius = 0.5;

nSteps = ceil(distance/radius);
wpts = [[linspace(startPoint(1),finishPoint(1),nSteps)]' [linspace(startPoint(2),finishPoint(2),nSteps)]'];

inputTrajectory = nan*ones(size(wpts));
inputTrajectory(1,:) = [0 0];
for i = 2:length(wpts)
   inputTrajectory(i,:) = wpts(i,:) - wpts(i-1,:);
end

trajectory = fcn_BoundedAStar_simulateIndividualTrajectory(startPoint, finishPoint, inputTrajectory, windFieldX, windFieldY, windFieldU, windFieldV, (fig_num));

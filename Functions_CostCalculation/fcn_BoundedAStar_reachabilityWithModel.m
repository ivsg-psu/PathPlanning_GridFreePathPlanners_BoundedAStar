function [reachableSet, cellArrayOfIntermediateCalculations, newStates] = fcn_BoundedAStar_reachabilityWithModel(costRad, windFieldU, windFieldV, windFieldX, windFieldY, modelParams, varargin)
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % type, 1 row
%         fcn_DebugTools_checkInputsToFunctions(...
%             costRad, '1column_of_numbers',[1 1]);
    end
end
...
% Use aircraft model to determine radius based on mass and fuel states
newStates = fcn_INTERNAL_findNewStates(costRad, modelParams.Platform);

t = fcn_INTERNAL_backProp(modelParams.Platform, newStates, modelParams.route);
radius = modelParams.route.v_ftps*t/5280;
...
%% fcn_INTERNAL_getAtmosphere

function [T_C, rho_slgpft3] = fcn_INTERNAL_getAtmosphere(alt_ft)
    % all properties from https://aerospaceweb.org/design/scripts/atmosphere/
    alt_vec = [0 5000 10000 20000 30000]; % altitude breakpoints (ft)
    T_vec = [15 5.0964 -4.8025 -24.586 -44.3506]; % air temp (C)
    rho_vec = [0.0023772 0.0020484 0.0017558 0.0012674 0.00089080]; % air density (slug/ft^3)
    % interpolate values
    T_C = interp1(alt_vec,T_vec,alt_ft);
    rho_slgpft3 = interp1(alt_vec,rho_vec,alt_ft);
end

%% fcn_INTERNAL_computeThrustReq
function Thrust_lbf = fcn_INTERNAL_computeThrustReq(Platform,route)
    % assume level flight (presented for simple model, but flight path
    % angle can be included if desired using basic relations). Calculations
    % are textbook.

    % compute dynamic pressure
    q = 0.5*Platform.Ambient.rho_slgpft3*route.v_ftps^2;

    % determine required coefficient of lift
    CL = (Platform.Airframe.emptyWeight + Platform.MassState)/(q*Platform.Airframe.Sref);

    % given coefficient of lift, use drag polar to compute coefficient of
    % drag
    CD = Platform.Airframe.CDv + Platform.Airframe.k*(CL - Platform.Airframe.CLv)^2;

    % thrust required is now simple to compute (again keeping level flight
    % assumption)
    Thrust_lbf = CD*q*Platform.Airframe.Sref;
    
end

%% fcn_INTERNAL_computeFuelConsumption
function mdot_lbmps = fcn_INTERNAL_computeFuelConsumption(thrust_lbf,~,~)
    % In reality, our engine model takes into account impacts of altitude,
    % and nonliearities with throttle PLA command. However, we can still
    % preserve the dependency of the altitude/velocity simply by using a
    % constant TSFC to compute our fuel consumption rate, because the
    % thrust required also depends on those same factors.

    % Some back of the envelope calculations:
    % platform endurance: 27 hours = 97200 sec https://en.wikipedia.org/wiki/General_Atomics_MQ-9_Reaper#Specifications
    % with 4000 lbm of thrust, average consumption rate is 0.0412 lbm/s
    % assuming that the average mass of the aircraft is at half fuel
    % capacity over this entire mission (2000lbm), from functions here it can be
    % found that the thrust required is ~840 lbf
    % therefore, TSFC = mdot/thrust = 0.0412/840 = 4.8991e-05 lbm/(sec-lbf)
    mdot_lbmps = thrust_lbf*4.8991e-05; % lbm/s
end

%% fcn_INTERNAL_findNewStates
function newStates = fcn_INTERNAL_findNewStates(endStepSize, Platform)
    % Find desired final endurance for thermal and fuel
    averageFuelBurnRate = 0.0412;
    currFE = Platform.MassState/averageFuelBurnRate;

    averageHeatingRate = 3;
    thermalStorageRemaining = Platform.MassState*Platform.TMS.cp_fuel_kJpkgK*(30 - Platform.TempState); % kJ
    currTE = thermalStorageRemaining/averageHeatingRate; 

    endurance = min(currTE,currFE);
    desTE = currTE - endStepSize;
    % NEED TO VALIDATE THIS RELATIONSHIP BUT IT WILL WORK FOR NOW
    desFE = currFE - (13/36)*endStepSize;

    % Back-calculate desired limiting state
    if desTE > desFE
        newStates.desMass = desFE*averageFuelBurnRate;
    else
        newStates.desMass = desFE*averageFuelBurnRate;
        newStates.desTemp = 30 - (desTE*averageHeatingRate)/(newStates.desMass*Platform.TMS.cp_fuel_kJpkgK);
    end
end

%% fcn_INTERNAL_backProp
function t = fcn_INTERNAL_backProp(Platform, newStates, route)
    [Platform.Ambient.T_C,Platform.Ambient.rho_slgpft3] = fcn_INTERNAL_getAtmosphere(route.alt_ft);
    
    % next compute the heat load
    Qload_kW = Platform.HeatLoad.(route.mode);

    % Compute required thrust
    Thrust_lbf = fcn_INTERNAL_computeThrustReq(Platform,route);

    % next compute the fuel consumption
    mdot_lbmps = fcn_INTERNAL_computeFuelConsumption(Thrust_lbf,Platform,route);

    t = (Platform.MassState - newStates.desMass)/mdot_lbmps;

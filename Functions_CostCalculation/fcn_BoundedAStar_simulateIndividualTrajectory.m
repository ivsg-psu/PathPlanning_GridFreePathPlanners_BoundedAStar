function trajectory = fcn_BoundedAStar_simulateIndividualTrajectory(startPoint, radius, windFieldX, windFieldY, windFieldU, windFieldV, randNum, varargin)
...
%
% FORMAT:
% [windVector] = fcn_BoundedAStar_simulateIndividualTrajectory(samplePoint, x, y, windFieldU, windFieldV, randNum, (fig_num))
%
% INPUTS:
%
%     startPoint: the [x, y] point to drop a particle into the wind field 
%
%     radius: the distance the vehicle can travel in one time step
%
...
%     wind velocity at each grid point
%
%     randNum: a 1x2 vector of random numbers used to choose the
%     orientation and % radius used for input 
%
%     (optional inputs)
%
...
headings = (2*pi)*inputInfo(:,1);
% dist = radius*inputInfo(:,2);
dist = radius;
genInput = dist*[cos(headings) sin(headings)];
...
for k = 1:numSteps
    ...
    windVector = fcn_BoundedAStar_sampleWindField(currentPoint, windFieldX, windFieldY, windFieldU, windFieldV, -1);
    disturbance = windVector;
    %disturbance = 0;
...
if flag_do_plots
    ...
    % Plot start point
    plot(startPoint(1), startPoint(2),'.','MarkerSize',15,'Color','black')

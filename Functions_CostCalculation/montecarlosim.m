% script_montecarlosim
% Tests: fcn_BoundedAStar_expandReachabilityWithWind

% general idea -- monte carlo-esque methods to show probability of
% reachable set estimation being correct. start at same position in same
% wind field, but use random selection methods to choose different inputs u
% from the set of allowable inputs U. 

% set of allowable inputs for our specific case is any input at any angle
% with a travel distance less than or equal to the specified maxiumum
% 'radius' (distance that can be traveled in a single time step).
...
%% DEMO case: simulate individual trajectories to estimate reachable set using monte carlo methods
figNum = 10001;
...
figure(figNum)
fcn_BoundedAStar_plotWindField(windFieldU,windFieldV,windFieldX,windFieldY,[],figNum);
hold on
plot(startPoints(1), startPoints(2),'.','MarkerSize',15,'Color','black');

for n = 1:nIter
    trajectories{n} = fcn_BoundedAStar_simulateIndividualTrajectory(startPoints,radius,windFieldY,windFieldX,windFieldU,windFieldV,randNum(n),[]);
    figure(figNum)
    %if trajectories{n}(end,1) < 10 && trajectories{n}(end,2) < 10
        plot(trajectories{n}(end,1),trajectories{n}(end,2),'.','Color','magenta','MarkerSize',10)
    %end
end

function [bound_polytopes,bound_box,bound_pts,all_bound]= fcn_BoundedAStar_calculateBoundingEllipsePolytopeBoundingBox(start,finish,polytopes,all_pts,bound_pts,offsetperp,offsetpara)
% fcn_BoundedAStar_calculateBoundingEllipsePolytopeBoundingBox creates a bounding box around 
% the straight line between A and B and then finds the points within the 
% bounding box and their corresponding polytopes
%
% [BOUND_POLYTOPES,BOUND_BOX,BOUND_PTS,ALL_BOUND]=fcn_BoundedAStar_calculateBoundingEllipsePolytopeBoundingBox(START,FINISH,POLYTOPES,ALL_PTS,BOUND_PTS,OFFSETPERP,OFFSETPARA)
% returns:
...
%      offsetperp=[15 15];
%      offsetpara = [2 2];
%      [bound_polytopes,bound_box,bound_pts,all_bound]= fcn_BoundedAStar_calculateBoundingEllipsePolytopeBoundingBox(start,finish,shrunk_polytopes,all_pts,bound_pts,offsetperp,offsetpara);
%      fcn_BoundedAStar_plotPolytopes(shrunk_polytopes,100,'r-',2,[-5 105 0 100],'square')
%      plot([start(1) finish(1)],[start(2) finish(2)],'k--','linewidth',2)
%      fcn_BoundedAStar_plotPolytopes(bound_polytopes,100,'b-',2)
...
% fcn_BoundedAStar_plotPolytopes(comb_polytopes,101,'r-',2,[0 200 0 200],'square')
% plot([start(1) finish(1)],[start(2) finish(2)],'k--','linewidth',2)
% fcn_BoundedAStar_plotPolytopes(bound_polytopes,101,'b-',2)

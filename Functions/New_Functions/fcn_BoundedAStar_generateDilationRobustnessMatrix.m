function [dilation_robustness_matrix] = fcn_BoundedAStar_generateDilationRobustnessMatrix(all_pts, start, finish, vgraph, mode, polytopes)
...
for i = 1:num_edges
    ...
    if 1==flag_do_debug 
        % Plot the visibility graph input
    ...
    if strcmp(mode, "2d") || strcmp(mode,"2D")
        primary_edge_startPoint = all_pts(edge_start_idx(i),1:2);
        primary_edge_endPoint = all_pts(edge_end_idx(i),1:2);
    elseif strcmp(mode, "3d") || strcmp(mode,"3D")
        primary_edge_startPoint = all_pts(edge_start_idx(i),1:3);
        primary_edge_endPoint = all_pts(edge_end_idx(i),1:3);
    else
    ...
    primary_edge_dir = primary_edge_endPoint - primary_edge_startPoint;
    ...
    secondary_edge_dirs = secondary_edge_ends - primary_edge_startPoint;
    ...
    primary_edge_mid_point = primary_edge_startPoint + 0.5*primary_edge_dir; % find the middle of the edge

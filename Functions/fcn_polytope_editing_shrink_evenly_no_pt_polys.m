...
%         end
    ...
    if shrinkable == 1 % if the obstacle was shrunk add it to shrunk_polytopes, otherwise remove polytope

        % init polytope
        shrunk_polytopes(cnt_poly) = polytopes(poly);

        % assign shrunk polytope to shrunk_polytopes struct
        shrunk_polytopes(cnt_poly).xv = xv'; % keep vertices seperate for easier calculations
        shrunk_polytopes(cnt_poly).yv = yv';
        shrunk_polytopes(cnt_poly).vertices = [[xv; xv(1)] [yv; yv(1)]]; % repeat first vertice for easy plotting
        [Cx,Cy,shrunk_polytopes(cnt_poly).area] = fcn_polytope_calculation_centroid_and_area([xv; xv(1)],[yv; yv(1)]);
        shrunk_polytopes(cnt_poly).mean = [Cx, Cy]; % calculate the polytope mean

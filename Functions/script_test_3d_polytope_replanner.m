...
num_pts_old = size(vgraph,1);
num_new_pts = size(route_new_points,1);
new_pts_idx = (num_pts_old+1):1:(num_pts_old+num_new_pts);
new_pts = [route_new_points new_pts_idx']; % add pt ID column to all_pts
all_pts_with_ids_no_start_and_fin = [verts_with_ids; new_pts];

%% add points along old route to new vgraph
new_vgraph = fcn_visibility_graph_3d_add_points(verts, start, finish, all_surfels, speed_limit, route_new_points, vgraph);
...
%% plan route
[cost, route] = fcn_algorithm_Astar3d(new_vgraph, all_pts_with_ids_no_start_and_fin, start_with_ids, finish_with_ids, new_rgraph);

...
% for i = 1:20 % sim repeat trials loop
outer_loop = tic;
if flag_do_plot
    %% plot the map
...
inner_time = toc(inner_loop);
inner_loops = [inner_loops, inner_time];
...
outer_time = toc(outer_loop);
outer_loops = [outer_loops, outer_time];
...
if threadpulling
    second_passes = [];
    inner_second_passes = [];
    for i = 1:20 % threadpulling repeat trials loop
    second_pass = tic;
    ...
    second_pass_time = toc(second_pass);
    second_passes = [second_passes, second_pass_time];
    inner_second_passes = [inner_second_passes, second_pass_inner];
    end % end threadpulling repeated trials loop
    
end
% end % end outer simulation repead loop

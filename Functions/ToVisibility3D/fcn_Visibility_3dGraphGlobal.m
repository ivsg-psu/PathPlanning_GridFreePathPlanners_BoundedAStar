...
%
% FORMAT:
% vgraph = fcn_Visibility_3dGraphGlobal(verts, start, finish, all_surfels, speed_limit, time_space_polytopes, dt, (figNum))
%
% INPUTS:
%
...
% REVISION HISTORY:
%
% As: fcn_visibility_graph_3d_global
% 2023, summer by Steve Harnett
% -- first write of function
%
% As: fcn_Visibility_3dGraphGlobal
...
% 2025_11_05 - S. Brennan, sbrennan@psu.edu
% -- updated this revision history to show naming
...
Nintersects = length(intersects_idx);
updateInterval = round(Nintersects/100);
nextUpdate = updateInterval;
for kth_intersection = 1:1:Nintersects
    if kth_intersection>nextUpdate
        waitbar(kth_intersection / Nintersects, h_waitBar, sprintf('Processing edge %d of %d...', kth_intersection, Nintersects));
        nextUpdate = nextUpdate + updateInterval;
    end
    i = intersects_idx(kth_intersection);
...
h_waitBar = waitbar(0, 'Processing edges for speed violations...');
Nviolations = length(speed_violation_idx);
updateInterval = round(Nintersects/100);
nextUpdate = updateInterval;

for lth_edge = 1:1:Nviolations

    if lth_edge>nextUpdate
        waitbar(lth_edge / Nviolations, h_waitBar, sprintf('Processing edge %d of %d...', lth_edge, Nviolations));
        nextUpdate = nextUpdate + updateInterval;
    end


    i = speed_violation_idx(lth_edge);
...
updateInterval = round(num_1s/100);
nextUpdate = updateInterval;

h_waitBar = waitbar(0, 'Processing edges...');
for ith_edge = 1:num_1s
    if ith_edge>nextUpdate
        waitbar(ith_edge / num_1s, h_waitBar, sprintf('Processing edge %d of %d...', ith_edge, num_1s));
        nextUpdate = nextUpdate + updateInterval;
    end

    start_pt = all_pts(rows_of_1s(ith_edge),1:3);
    end_pt = all_pts(cols_of_1s(ith_edge),1:3);
    ...
    while p <= num_polys
        ...
        if is_in && ~ is_on
            vgraph(rows_of_1s(ith_edge),cols_of_1s(ith_edge)) = 0;
            p = num_polys+1;
        end
        % if not, continue
        p = p + 1;

function [visibility_matrix_new, all_pts_new, start_new, finish_new] = fcn_visibility_modify_visibility_graph(visibility_matrix, all_pts, start, finish, polytopes_after, polytope_of_interest, mode)
    ...
    [xtf, xloc] = ismember(polytope_vertices(:,1), all_pts(:,1));
    [ytf, yloc] = ismember(polytope_vertices(:,2), all_pts(:,2));
    idx_of_points_on_polytope = union(xloc,yloc);
    pts_on_polytope = all_pts(idx_of_points_on_polytope,:);
    ...
    if strcmp(mode,'remove')
        ...
        isInside = fcn_MapGen_isCrossingAABB(AABB,all_pts_new);

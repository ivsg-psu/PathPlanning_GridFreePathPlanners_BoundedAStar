% script_test_fcn_BoundedAStar_calculateBoundingEllipsePolytope
% tests fcn_BoundedAStar_calculateBoundingEllipsePolytope
...
figure(fig_num); clf;
 ...
 % Generate polytope field
 polytopes = fcn_MapGen_generatePolysFromSeedGeneratorNames('haltonset', [1, 100], [0 0 100 100],[],47474);
 ...
 if 1==1
     ...
     all_pts = fcn_VGraph_polytopesGenerateAllPtsTable(shrunk_polytopes, start(1:2), finish(1:2), -1);
 else
     % % OLD:
 ...
 offsetperp=[15 15];
 offsetpara = [2 2];
 [bound_polytopes,bound_box,bound_pts,all_bound]= fcn_BoundedAStar_calculateBoundingEllipsePolytope(start,finish,shrunk_polytopes,all_pts,bound_pts,offsetperp,offsetpara,fig_num);

sgtitle(titleString, 'Interpreter','none');

% Check variable types
assert(isstruct(bound_polytopes));

% Check variable sizes
Npoly = 37;
assert(isequal(Npoly,length(bound_polytopes))); 

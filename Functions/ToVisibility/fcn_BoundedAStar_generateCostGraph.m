...
%
% A function for generating a cost matrix and heuristic cost vector.  The
% cost matrix describes the actual cost to go from one point to another in
% a map.  The heuristic vector describes the estimated cost of going from
% each point to the goal.  If there are multiple goals, this is the minimum
% of going to any goal because the heuristic should be an underestimate of
% the actual cost (see:
% http://theory.stanford.edu/~amitp/GameProgramming/Heuristics.html).  Note
% using this function is completely optional.  It is commonly used prior to
% calling a planner such as Astar to conveniently generate a cost map but
% it may be circumvented if the user wishes to generate their own cost
% function and cost function matrix
%
%
% FORMAT:

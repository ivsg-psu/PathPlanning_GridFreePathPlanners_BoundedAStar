...
% 2025_10_08 - K. Hayes
% -- replaced calls to fcn_plot_polytopes with fcn_MapGen_plotPolytopes
% -- added call to fcn_BoundedAStar_polytopesGenerateAllPtsTable
% -- replaced calls to fcn_bounding_ellipse_min_perim_path with
...
% - Removed cl+c and c+lear commands
...
fcn_MapGen_plotPolytopes(shrunk_polytopes, plotFormat, [1 0 0 1 0.5], fig);
...
xings=fcn_Visibility_linePolytopeIntersections(xiP,yiP,xiQ,yiQ,xjP,yjP,D,di,num_int,shrunk_polytopes, -1);
...
plotFormat.Color = 'Blue';
plotFormat.LineWidth = 2;
fcn_MapGen_plotPolytopes(shrunk_polytopes, plotFormat, [1 0 0 1 0.5], 9998);
plot([start(1) finish(1,1)],[start(2) finish(1,2)],'kx','linewidth',1)
...
max_dist=fcn_BoundedAStar_calculateBoundingEllipseMinPerimPath(xing_polytopes,xings,start,finish,-1);
...
[bound_polytopes,bound_box,bound_pts]=...
    fcn_BoundedAStar_calculateBoundingEllipsePolytope...
    (start,finish,shrunk_polytopes,all_pts,bound_pts,perp_offset,para_offset, -1);
% plot the results with the bound polytopes plotted in red and the bound
% points plotted as green circles.
plotFormat.Color = 'Blue';
plotFormat.LineWidth = 2;
fcn_MapGen_plotPolytopes(shrunk_polytopes, plotFormat, [1 0 0 1 0.5], 9997);
...
plotFormat.LineWidth = 2;
fcn_MapGen_plotPolytopes(bound_polytopes,plotFormat, [0 0 0 0 0], 9997);
...
A.x = start(1), A.y = start(2), B.x = finish(1), B.y = finish(2);
...
plotFormat.Color = 'Blue';
plotFormat.LineWidth = 2;
fcn_MapGen_plotPolytopes(shrunk_polytopes, plotFormat, [1 0 0 1 0.5], 9996);

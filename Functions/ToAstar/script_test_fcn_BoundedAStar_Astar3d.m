% script_test_fcn_BoundedAStar_Astar3d
% tests fcn_BoundedAStar_Astar3d
...
% all_pts = fcn_BoundedAStar_polytopesGenerateAllPtsTable(time_space_polytopes, start_xy, finish_xy);

all_pts = [];
for poly = 1:length(time_space_polytopes)
    all_pts = [all_pts; time_space_polytopes(poly).dense_vertices];
end

% create vgraph
[vgraph, vres] = fcn_Visibility_clearAndBlockedPointsGlobal(time_space_polytopes, starts, finish, -1);

% create cost graph and heuristic vector
mode = 'xyz or xyt';

function [bugCrawlDistance] = fcn_BoundedAStar_calculateBoundingEllipseMinPerimPath(intersectedPolytopes, intersections, startPointData, finishPointData, varargin)
% fcn_BoundedAStar_calculateBoundingEllipseMinPerimPath
%
% Adds up all the distances between intersections along the start and
% finish using the "bug" algorithm. Distances are found around obstacles
% using the shortest perimeter distance around each intersected obstacle.
%
% FORMAT:
%
% [bugCrawlDistance] = fcn_BoundedAStar_calculateBoundingEllipseMinPerimPath(...
%    intersectedPolytopes, intersections, startPointData, finishPointData,(figNum))
%
% INPUTS:
%
%   intersectedPolytopes: a 1-by-p seven field structure of intersected polytopes,
...
%
%   startPointData:  a 1-by-5 vector of starting point information, including:
%       x-coordinate
%       y-coordinate
%       point id number
%       obstacle id number
...
%
%   finishPointData: a 1-by-5 vector of ending point information, including the same
...
%
% OUTPUTS:
%   bugCrawlDistance: sum of all the distances between intersections and along
%   minimum perimeters
%
% DEPENDENCIES:
%
...
% Revision history:
% As: fcn_bounding_ellipse_min_perimeter_path
% 2018_11_17 by Seth Tau
% - first write of the code
%
% As: fcn_BoundedAStar_calculateBoundingEllipseMinPerimPath
...
% 2025_07_25 - S. Brennan, sbrennan@psu.edu
% - Cleaned up documentation
% - Removed extra figure call in Inputs area
% - Cleaned up variables for consistency and clarity
%   % * fig_num to figNum
%   % * int_polytopes to intersectedPolytopes
%   % * start to startPointData
%   % * finish to finishPointData
%   % * max_dist to bugCrawlDistance
...
try % try block the entire function so if it fails for being inside a polytope, we can just assume
    ...
    %% starting values
    bugCrawlDistance = 0; % start the bugCrawlDistance at 0
    ...
    startpt = startPointData(1:2);
    startobs = 0; % default value
    for obs = 1:size(intersectedPolytopes,2)
        [~,on] = inpolygon(startPointData(1),startPointData(2),intersectedPolytopes(obs).vertices(:,1),intersectedPolytopes(obs).vertices(:,2));
    ...
    finishpt = finishPointData(1:2);
    finishobs = 0; % default value
    for obs = 1:size(intersectedPolytopes,2)
        [~,on] = inpolygon(finishPointData(1),finishPointData(2),intersectedPolytopes(obs).vertices(:,1),intersectedPolytopes(obs).vertices(:,2));
        if on % if we start on an obstacle we intersect
            uobs = unique(intersections.obstacles); % intersected obstacles with no repeats
    ...
    while ~isempty(points) % points not empty
        if startobs ~= 0 && bugCrawlDistance == 0 % starting on an obstacle
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%
            bugCrawlDistance = bugCrawlDistance + min_dist; % add the distance to the bugCrawlDistance
            xing1 = points(ind,:); % set closest point as first perimeter point
            polytope = intersectedPolytopes(ind); % polytope of interest
            intersectedPolytopes(ind) = []; % remove polytope
        ...
        intersectedPolytopes(rem_ind) = []; % remove polytope
        ...
        %             [in,on] = inpolygon(startpt(1),startpt(2),intersectedPolytopes(ind).xv,intersectedPolytopes(ind).yv);
        ...
        %                 bugCrawlDistance = bugCrawlDistance + min_dist; % distance to reach that point
        ...
        %             xing1 = startpt;
        %             polytope = intersectedPolytopes(startobs);
        %             curobs = startobs;
        %         else
        %             bugCrawlDistance = bugCrawlDistance + min_dist; % add the distance to the bugCrawlDistance
        %             xing1 = points(ind,:); % set closest point as first perimeter point
        ...
        % add the smallest perimeter to the bugCrawlDistance
        if perimeter1 < perimeter2
            bugCrawlDistance = bugCrawlDistance + perimeter1;
        else
            bugCrawlDistance = bugCrawlDistance + perimeter2;
        end
        startpt = xing2; % make xing2 the next starting point
    end
    % add the distance from the last intersection to the end point
    bugCrawlDistance = bugCrawlDistance + sum((startpt - finishpt).^2,2).^0.5;
    ...
    % inside a polytope
    bugCrawlDistance = 2*sum((startpt - finishpt).^2,2).^0.5;
end

...
% - Finish formatting this script to include fastmode tests, testing area,
% etc.
...
[clear_pts,blocked_pts,D,di,dj,num_int,xiP,yiP,xiQ,yiQ,xjP,yjP,xjQ,yjQ] = fcn_VGraph_clearAndBlockedPoints(shrunk_polytopes,start,finish);
...
intersectedPolytopes = shrunk_polytopes(intersections(end).obstacles);

% Call function to calcluate the minimum perimeter path (?)
max_dist=fcn_BoundedAStar_calculateBoundingEllipseMinPerimPath(intersectedPolytopes,intersections,start,finish,(fig_num));
...
% fillFormat = [1 0 0 0 0.5];
fillFormat = [];
fcn_MapGen_plotPolytopes(intersectedPolytopes, (plotFormat),(fillFormat),(fig_num));
